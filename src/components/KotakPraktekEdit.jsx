...
import { editByIdPraktek } from "../features/practiceSlice";
...
const KotakPraktekEdit = ({ data }) => {
  ...
  const [spesialis, setSpesialis] = useState(data?.spesialis[0]);
  const [harga, setHarga] = useState(data?.harga);
  const [hargaPromo, setHargaPromo] = useState(data?.harga_promo);
  const { message: messagePractice, isSuccess } = useSelector(
  ...
  useEffect(() => {
    if (messagePractice == "Data berhasil diubah") {
      navigate("/praktek");
